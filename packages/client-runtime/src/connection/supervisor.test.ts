...
describe("EnvironmentSupervisor", () => {
  ...
  it.effect("reconnects immediately when a mobile foreground liveness probe fails", () =>
    Effect.gen(function* () {
  ...
  it.effect("uses the full tolerance window for a stalled desktop foreground probe", () =>
    Effect.gen(function* () {
      ...
      expect((yield* SubscriptionRef.get(supervisor.state)).phase).toBe("connected");
      yield* TestClock.adjust("1 milli");
      // The first stalled desktop probe exceeds the connection probe timeout
      // and is kept as an isolated transient failure; a second wake with only
      // the stale probe would reconnect, but nothing further arrives.
      expect(yield* Ref.get(harness.sessionCount)).toBe(1);
      expect((yield* SubscriptionRef.get(supervisor.state)).phase).toBe("connected");
    }).pipe(Effect.provide(TestClock.layer())),
  );
  ...
  it.effect("reconnects immediately when the watchdog confirmation also fails", () =>
    Effect.gen(function* () {
      const probeCount = yield* Ref.make(0);
      const harness = yield* makeHarness({
        probe: (attempt) =>
      ...
      yield* awaitState(supervisor.state, (state) => state.phase === "connected");
      yield* harness.wake("connection-watchdog-probe");
      // The first watchdog probe confirms by re-probing; the confirmation
      // also fails, so the failed wake probe reconnects immediately without
      // sleeping the first backoff rung.
      yield* awaitState(
        supervisor.state,
        (state) => state.phase === "connected" && state.generation === 2 && state.attempt === 1,
      );
  ...
  it.effect("reconnects immediately after consecutive foreground liveness probe failures", () =>
    Effect.gen(function* () {
      const probeCount = yield* Ref.make(0);
      const firstProbeCalled = yield* Deferred.make<void>();
      const secondProbeCalled = yield* Deferred.make<void>();
  ...
  it.effect("reconnects immediately after consecutive stalled foreground liveness probes", () =>
    Effect.gen(function* () {
      const probeCount = yield* Ref.make(0);
      const firstProbeStarted = yield* Deferred.make<void>();
