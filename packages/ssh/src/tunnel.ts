...
const REMOTE_ARCHIVE_RUNNER_SCRIPT = `#!/bin/sh
set -eu
@@T3_NODE_ENV_SCRIPT@@
...
const REMOTE_PACKAGE_RUNNER_SCRIPT = `#!/bin/sh
set -eu
@@T3_NODE_ENV_SCRIPT@@
T3_NODE_SCRIPT_PATH=@@T3_NODE_SCRIPT_PATH@@
...
export function buildRemoteT3RunnerScript(input?: RemoteT3RunnerOptions): string {
  ...
  const hasPackageRunner =
    Boolean(input?.packageSpec) ||
    Boolean(input?.localPackageArchivePath) ||
    Boolean(input?.packageCacheDir) ||
    requireExactBuild;

  if (nodeScriptPath === "" && archiveVersion === "" && !hasPackageRunner) {
  ...
  const runnerTemplate =
    archiveVersion !== "" || (!hasPackageRunner && nodeScriptPath !== "")
      ? REMOTE_ARCHIVE_RUNNER_SCRIPT
      : REMOTE_PACKAGE_RUNNER_SCRIPT;

  return stripTrailingNewlines(
    applyScriptPlaceholders(runnerTemplate, {
...
export function buildRemotePairingScript(
  ...
  const runnerScript = input ? buildRemoteT3RunnerScript(input) : null;
  const runnerInit = runnerScript
    ? `cat >"$RUNNER_FILE" <<'SH'\n${stripTrailingNewlines(runnerScript)}\nSH\nchmod 700 "$RUNNER_FILE"`
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  ...
  > {
    ...
    const result = yield* runSshCommand(target, {
      ...
      timeoutMs: preparedRunner?.localPackageArchivePath?.trim()
        ? REMOTE_PACKAGE_READY_TIMEOUT_MS + 30_000
        : isNodeScriptRunner(preparedRunner)
...
> {
  ...
  const result = yield* runSshCommand(target, {
    ...
    ...(runner && !isNodeScriptRunner(runner)
