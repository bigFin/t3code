...
function ThreadNavigationSidebarPane(
  ...
  const environmentScopedPendingTasks = useMemo(
    () =>
      pendingTasks.filter((pendingTask) =>
        isEnvironmentInHomeScope(options.selectedEnvironmentIds, pendingTask.environmentId),
      ),
  ...
  const scopedProjects = useMemo(
    ...
    [environmentScopedProjects, selectedProjectRefs, threadListV2Enabled],
  );
  const scopedThreads = useMemo(
    ...
    [environmentScopedThreads, selectedProjectRefs, threadListV2Enabled],
  );
  const scopedPendingTasks = useMemo(
    ...
    [environmentScopedPendingTasks, selectedProjectRefs, threadListV2Enabled],
  );
  ...
  // PR states stream in per-row. The v2 partition applies the configured
  ...
  const handleChangeRequestState = useCallback(
    (threadKey: string, changeRequest: ChangeRequestSettleSource | null) => {
      setChangeRequestByKey((current) => {
        ...
        if (changeRequest === null) next.delete(threadKey);
        else next.set(threadKey, changeRequest);
        return next;
      });
    },
    [],
