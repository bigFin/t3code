 ...
 *     block below configures one instance of every configured driver
...
describe("ProviderInstanceRegistryLive — all drivers slice", () => {
  ...
  // Path). OpenCode and Pi additionally yield their provider-specific
  // runtime services at construction time, so both live layers are wired
  // into the stack. OpenCode bundles its own `NetService.layer`; the
  // remaining process dependencies are resolved by piping the merged
  // runtime layer through `provideMerge(NodeServices.layer)`.
  //
  ...
  it.live("boots one instance of every configured driver from a single config map", () =>
    Effect.gen(function* () {
