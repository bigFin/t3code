...
const LEGACY_CODEX_MODELS = new Set([
  "gpt-5",
  "gpt-5-codex",
  "gpt-5.2",
  "gpt-5.3",
...
export function isLegacyCodexModel(model: string): boolean {
  // Codex model/list also returns entries from user-authored model catalogs,
  // but does not identify which entries are built in. Keep the legacy list
  // explicit so custom and future model slugs remain visible by default.
  return LEGACY_CODEX_MODELS.has(model);
}
...
const probeCodexAppServerProvider = Effect.fn("probeCodexAppServerProvider")(function* (input: {
  ...
  const [skillsResponse, models, rateLimitsResult] = yield* Effect.all(
    [
      ...
      client.request("account/rateLimits/read", undefined).pipe(
        Effect.map((response) => ({
          ...
          usage: mapCodexRateLimits(response),
        })),
  ...
  const usage = rateLimitsResult.usage;
