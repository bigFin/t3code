...
const make = Effect.gen(function* () {
  ...
  const processTurnInterruptRequested = Effect.fn("processTurnInterruptRequested")(function* (
    ...
    // Carry the orchestration turn id when the request names one. Adapters
    // that track provider turns (e.g. Codex) scope the interrupt to it;
    // others interrupt by session.
    yield* providerService
      .interruptTurn({
        threadId: event.payload.threadId,
            ...
            session: {
              ...
              ...(thread.session?.nativeSession
                ? {
                    nativeSession: {
                      ...thread.session.nativeSession,
                      ...(event.payload.releaseToCli === true
...
// Subscribe eagerly so no events are dropped between start-up and the
    ...
    yield* forkParked(
      Stream.runForEach(domainEvents, processEvent).pipe(
        Effect.onExit((exit) =>
