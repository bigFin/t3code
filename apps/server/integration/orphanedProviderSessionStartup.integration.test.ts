...
const startupDependencies = Layer.mergeAll(
  ...
  Layer.succeed(ProviderService.ProviderService, {
    ...
    reattachSession: () => Effect.die("unused"),
  ...
  (restart) =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        Effect.provide(
          // Feed the persisted outputs into the shared deps: the fork's session
          // importers inside startupDependencies require the engine/snapshot
          // services that mergeAll cannot wire intra-merge.
          startupDependencies.pipe(
            Layer.provideMerge(
              Layer.mergeAll(makePersistedRuntimeLayer(config.dbPath), ServerSettings.layerTest()),
            ),
          ),
        ),
      );
