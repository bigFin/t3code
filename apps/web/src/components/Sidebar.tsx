        ...
        <TooltipTrigger
          render={
            <MenuTrigger
              ...
              className="inline-flex h-7 min-w-7 cursor-pointer items-center justify-center rounded-md text-icon-muted transition-colors hover:bg-accent hover:text-foreground focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring"
            />
          }
        >
...
export default function Sidebar() {
  ...
  const toggleHostGroup = useCallback(
    (environmentId: EnvironmentId) =>
      setCollapsedHostGroups((collapsed) =>
  ...
  const toggleHostInactiveFold = useCallback(
    (environmentId: EnvironmentId) =>
      setExpandedInactiveHosts((expanded) =>
  ...
  const sidebarListItems = useMemo((): readonly SidebarListItem[] => {
    ...
    if (groupedHostView) {
      return orderedThreads.map((thread) => {
        const key = scopedThreadKey(scopeThreadRef(thread.environmentId, thread.id));
        return { kind: "thread", key, section: sectionByThreadKey.get(key)! };
      });
    }
          ...
          <SidebarGroup className="z-[1]">
            ...
            {hostOptions.length > 1 ? (
              <div
                ...
                className="flex min-w-0 gap-1 overflow-x-auto px-0.5 pb-0.5 pt-1"
              >
                <button
                  type="button"
                    ...
                    {(() => {
                      ...
                      if (groupedHostView) {
                        ...
                        for (const group of hostThreadGroups) {
                          const expanded = !collapsedHostGroups.includes(group.environmentId);
                          items.push(
                            <li
                          ...
                          for (const thread of group.threads) {
                            ...
                            if (visibleKeys.has(key)) items.push(renderThreadRow(thread, section));
                          }
                        }
                        return items;
                      }
