...
describe("sortThreadsForSidebar", () => {
  ...
  it("orders by creation time when that preference is selected", () => {
  ...
  it("orders by latest user message when recent activity is selected", () => {
    const sorted = sortThreadsForSidebar(
      [
        sortable({
          id: "newest-created",
    ...
    expect(sorted.map((thread) => thread.id)).toEqual(["recently-active", "newest-created"]);
  });
