...
/**
 * Preloads the user-supplied sprite sheet so rendering never flashes a
 * half-loaded background image. Returns null until the URL decodes.
 */
function useSpriteUrl(spriteUrl: string): string | null {
...
export function FluxCompanion() {
  ...
  const spriteUrl = useClientSettings((settings) => settings.companionSpriteUrl);
  const loadedSpriteUrl = useSpriteUrl(spriteUrl);
  ...
  if (!enabled || !finePointer || loadedSpriteUrl === null) {
    return null;
  }
