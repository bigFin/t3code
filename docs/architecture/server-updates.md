...
flowchart TD
    ...
    J --> K
    K --> L[Install and run version preflight]
    L -->|fails| M[Remove failed runtime and keep current server]
    L -->|passes| N{Handoff method}
    N -->|boot-service| O[Rewrite and restart T3 systemd unit]
    N -->|respawn| P[Start delayed replacement and exit current process]
    O --> Q[Reconnect with fresh backoff]
    P --> Q
    Q --> R[Replacement publishes ready at target version]
```
