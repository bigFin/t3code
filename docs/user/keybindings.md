...
`thread.latestCompleted` opens the thread with the newest completed assistant response and defaults
to `mod+alt+[`. `thread.nextWorking` cycles through threads whose agents are actively working and
defaults to `mod+alt+]`.
